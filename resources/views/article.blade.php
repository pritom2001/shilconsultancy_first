...
@section('content')
            ...
            <div class="w-full lg:w-2/3">
                <article class="card-bg rounded-xl p-6 md:p-10">
                    <header class="mb-8">
                        ...
                        <div class="mt-4 flex items-center text-sm text-gray-500">
                            <!-- FIXED: Author display -->
                            <span>By
                                {{ is_object($blog->author) ? $blog->author->name ?? 'Admin' : $blog->author ?? 'Admin' }}</span>
                ...
                <div class="card-bg rounded-xl p-6 md:p-8 mt-8 flex items-start sm:items-center flex-col sm:flex-row">
                    <div
                        ...
                        <!-- FIXED: Author initial -->
                        {{ is_object($blog->author) ? substr($blog->author->name ?? 'A', 0, 1) : substr($blog->author ?? 'A', 0, 1) }}
                    </div>
                    <div>
                        <!-- FIXED: Author name -->
                        <h4 class="text-xl font-bold text-white">About
                            {{ is_object($blog->author) ? $blog->author->name ?? 'Admin' : $blog->author ?? 'Admin' }}
                        </h4>
                ...
                <div class="card-bg rounded-xl p-6 md:p-8 mt-8">
                    ...
                    <form action="/articles/{{ $blog->slug }}/comment" method="POST">
                        @csrf
            ...
            <aside class="w-full lg:w-1/3 space-y-8">
                ...
                <div class="card-bg rounded-xl p-6">
                    ...
                    <ul class="space-y-4">
                        @foreach ($latestPosts as $latest)
                            <li><a href="{{ route('public.blogs.show', $latest->slug) }}"
                ...
                <div class="card-bg rounded-xl p-6">
                    ...
                    <h4 class="font-bold text-lg text-white hover:text-cyan-400 transition-colors">
                        <a href="{{ route('public.blogs.show', $featuredPost->slug) }}">{{ $featuredPost->title }}</a>
                    </h4>
    ...
    <style>
        ...
        .gradient-text {
            background: linear-gradient(90deg, #6e45e2, #88d3ce, #ff7e5f);
